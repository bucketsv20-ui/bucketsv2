...
create table if not exists season_settings (
  season_id bigint primary key references seasons(season_id) on delete cascade,
...
begin
  if not exists (
    ...
    create policy season_settings_select_admin on season_settings
      for select
      using (is_admin());
  end if;
  ...
  if not exists (
    ...
    create policy season_settings_write_admin on season_settings
      for all
      using (is_admin())
      with check (is_admin());
  end if;
